...
public static class Solver
{
    ...
    public static object Part2(Data data)
    {
        ...
        var points = new HashSet<(int x, int y)>();
        var steps = 0;
        points.Add((data.End.x, data.End.y));
        ...
        while(points.Count > 0)
        {
            ...
            foreach (var (x, y) in points)
            {
                ...
                if (x > 0 && data.Grid[x, y] - data.Grid[x - 1, y] <= 1 && grid[x - 1, y] < 0)
                    newPoints.Add((x - 1, y));
                if (x < width - 1 && data.Grid[x, y] - data.Grid[x + 1, y] <= 1 && grid[x + 1, y] < 0)
                    newPoints.Add((x + 1, y));
                if (y > 0 && data.Grid[x, y] - data.Grid[x, y - 1] <= 1 && grid[x, y - 1] < 0)
                    newPoints.Add((x, y - 1));
                if (y < height - 1 && data.Grid[x, y] - data.Grid[x, y + 1] <= 1 && grid[x, y + 1] < 0)
                    newPoints.Add((x, y + 1));
            }
